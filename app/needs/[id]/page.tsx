    ...
    <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-12">
      ...
      {claimed ? (
        <p className="rounded-xl bg-[#e7f2ea] px-4 py-3 text-sm text-[#2f4a3a]">
          Claim locked. Thank you — fulfill on the window, then recognition can follow.
        </p>
      ) : null}
      ...
      <header className="rounded-3xl border border-[#e7dcc8] bg-white p-8">
        ...
        <p className="mt-4 inline-flex rounded-full bg-[#f0e6d6] px-3 py-1 text-xs font-medium text-[#7a5c2e]">
          Status: {need.status}
        </p>
      </header>

      {need.status === "open" ? (
        <ClaimForm needId={need.id} />
      ) : (
        <section className="rounded-2xl border border-[#e7dcc8] bg-[#fffaf2] p-6 text-sm text-[#3d5346]">
          <h2 className="text-lg font-semibold text-[#2f4a3a]">Claimed</h2>
          {need.claimedBy ? (
